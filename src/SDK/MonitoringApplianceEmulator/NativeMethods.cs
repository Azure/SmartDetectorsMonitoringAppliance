...
namespace Microsoft.Azure.Monitoring.SmartDetectors.MonitoringApplianceEmulator
{
    ...
    public static class NativeMethods
    {
        ...
        internal static void RemoveIcon(Window window)
        {
            ...
            int result = SetWindowLong(hwnd, GwlExstyle, extendedStyle | WsExDlgmodalFrame);
            if (result == 0)
            {
                throw new InvalidOperationException($"Failed to set the window's icon with HRESULT={Marshal.GetLastWin32Error()}");
            }
