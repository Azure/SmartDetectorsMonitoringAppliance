...
namespace Microsoft.Azure.Monitoring.SmartDetectors.MonitoringAppliance.Trace
{
    ...
    public class ApplicationInsightsTracer : ITracer
    {
        ...
        private readonly Dictionary<string, string> customProperties;
        ...
        public ApplicationInsightsTracer(string sessionId, TelemetryConfiguration telemetryConfiguration, bool? sendVerboseTracesToAi = null, string buildVersion = null)
        {
            ...
            this.telemetryClient = CreateTelemetryClient(sessionId, telemetryConfiguration);
        ...
        {
            // Trace exception
            this.TrackTrace(exception.ToString(), SeverityLevel.Error);
        ...
        {
            return new Dictionary<string, string>(this.customProperties);
        }
        ...
        private static TelemetryClient CreateTelemetryClient(string sessionId, TelemetryConfiguration telemetryConfiguration)
        {
        ...
        {
            this.TrackTrace(message, severityLevel);
        }
        ...
        /// <param name="message">The message to trace</param>
        /// <param name="severityLevel">The message's severity level</param>
        private void TrackTrace(string message, SeverityLevel severityLevel)
        {
            var traceTelemetry = new TraceTelemetry(message, severityLevel);

            // Set telemetry properties and context
            this.SetTelemetryProperties(traceTelemetry);
            ...
            traceTelemetry.Properties[TraceOrderKey] = currentTraceOrder.ToString(CultureInfo.InvariantCulture);
        ...
        private Exception HandleTooLongException(Exception exception)
        {
            ...
            this.TrackTrace($"Exception was too long - reporting an ExceptionTooLongException with referenceId = {referenceId}", SeverityLevel.Error);
